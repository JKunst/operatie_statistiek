...
from download_logger import (
    log_download, haal_log_op, zoek_leerling,
    statistieken, controleer_wachtwoord, log_als_csv,
    verwijder_leerling, initialiseer_db,
)

# DB alvast initialiseren bij opstarten
initialiseer_db()
...
.dossier-wrapper::after {
    ...
    border: 1px solid rgba(139,90,43,0.25); pointer-events: none;
}
...
.stat-blok { background:#0a1628; border:1px solid #1e3a55; padding:1rem 1.2rem; text-align:center; }
.stat-blok .stat-num { font-family:'Oswald',sans-serif; font-size:2rem; font-weight:600; color:#7ab8d8; line-height:1; }
.stat-blok .stat-label { font-family:'Courier Prime',monospace; font-size:0.7rem; color:#4a7a9b; text-transform:uppercase; letter-spacing:0.1em; margin-top:0.3rem; }
.log-table { width:100%; border-collapse:collapse; font-family:'Courier Prime',monospace; font-size:0.82rem; }
.log-table th { background:#0a1628; color:#5a9abf; text-align:left; padding:0.5rem 0.75rem; font-family:'Oswald',sans-serif; font-size:0.65rem; letter-spacing:0.15em; text-transform:uppercase; border-bottom:2px solid #1e3a55; }
.log-table td { padding:0.5rem 0.75rem; border-bottom:1px solid #1a2e42; color:#b0c8d8; vertical-align:middle; }
.log-table tr:hover td { background:rgba(26,92,139,0.15); }
.ln-cell { font-size:1rem; font-weight:bold; letter-spacing:0.1em; color:#e0f0ff !important; }
.cnt-badge { background:#1a5c8b; color:white; padding:0.15rem 0.6rem; border-radius:10px; font-size:0.75rem; font-weight:bold; }
.cnt-meer { background:#8b3a1a !important; }
...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("pagina","intro"),("leerlingnummer",""),("dataset_gegenereerd",False),("docent_ingelogd",False)]:
...
def render_intro():
    ...
    st.markdown('<div class="form-label">Voer je leerlingnummer in (6 cijfers)</div>', unsafe_allow_html=True)

    col1, col2 = st.columns([3, 1])
    with col1:
        ln_input = st.text_input("ln", label_visibility="collapsed", placeholder="bijv. 144555", max_chars=6, key="ln_input_field")
    with col2:
    ...
    if st.session_state.dataset_gegenereerd and st.session_state.leerlingnummer:
        ...
        <div class="bevestiging-box">
            <strong>✓ Toegang verleend — Leerling {ln}</strong><br>
        ...
        with st.spinner("Dataset genereren..."):
            xlsx_bytes = genereer_xlsx_bytes(ln)
        ...
        st.download_button(
            ...
            kwargs={"leerlingnummer": ln, "arts_naam": arts},
        )
    ...
    # Verborgen docent-knop (subtiel, rechts uitgelijnd)
...
def render_docent_dashboard():
    ...
    st.markdown(f"""
    <div class="stat-grid">
        <div class="stat-blok"><div class="stat-num">{n_leerlingen}</div><div class="stat-label">Leerlingen</div></div>
        <div class="stat-blok"><div class="stat-num">{n_downloads}</div><div class="stat-label">Totaal downloads</div></div>
        <div class="stat-blok"><div class="stat-num">{n_artsen}/4</div><div class="stat-label">Artsen verdeeld</div></div>
    </div>
    """, unsafe_allow_html=True)

    # ── Verdeling per arts (balkjes) ──
    arts_bars = ""
    for arts in ARTSEN:
        cnt    = per_arts.get(arts["naam"], 0)
        kleur  = ARTS_KLEUREN.get(arts["naam"], "#aaa")
        breedte = max(3, int(cnt / max(n_leerlingen, 1) * 100))
        arts_bars += f"""
        <div style="margin-bottom:0.75rem;">
            <div style="display:flex;justify-content:space-between;align-items:center;
                        font-family:'Courier Prime',monospace;font-size:0.82rem;
                        color:#b0c8d8;margin-bottom:0.25rem;">
            ...
            <div style="background:#1e3a55;height:8px;border-radius:4px;overflow:hidden;">
                <div style="background:{kleur};height:8px;width:{breedte}%;"></div>
            </div>
        </div>"""

    leeg_msg = "" if n_leerlingen > 0 else '<p style="color:#4a7a9b;font-family:\'Courier Prime\',monospace;font-size:0.85rem;">Nog geen downloads geregistreerd.</p>'
    st.markdown(f'<div class="docent-card"><h3>Verdeling verdachte artsen</h3>{arts_bars}{leeg_msg}</div>', unsafe_allow_html=True)
    ...
    if log:
        ...
        for r in log:
            ...
            cnt   = r["downloads"]
            badge = f'<span class="cnt-badge{"" if cnt==1 else " cnt-meer"}">{cnt}×</span>'
            rijen += f"""<tr>
        ...
        <div class="docent-card">
            <h3>Gedownloade datasets ({n_leerlingen})</h3>
            <table class="log-table">
                <thead><tr>
                    <th>Leerlingnummer</th><th>Verdachte arts</th>
                    <th>Eerste download</th><th>Laatste download</th><th>Aantal</th>
                </tr></thead>
                <tbody>{rijen}</tbody>
            </table>
        </div>
        ...
        st.markdown("""
        <div class="docent-card">
            <h3>Gedownloade datasets</h3>
            <p style="color:#4a7a9b;font-family:'Courier Prime',monospace;font-size:0.85rem;">
                Nog geen downloads geregistreerd. Zodra een leerling op de downloadknop klikt, verschijnt die hier automatisch.
            </p>
        </div>
        """, unsafe_allow_html=True)

    # ── Leerlingnummer opzoeken ──
    ...
    with col1:
        zoek = st.text_input("Zoek", label_visibility="collapsed",
                             placeholder="Voer leerlingnummer in (6 cijfers)", max_chars=6, key="zoek_ln")
    with col2:
    ...
    if zoek and len(zoek) == 6 and zoek.isdigit():
        ...
        gevonden = zoek_leerling(zoek)   # SQLite lookup
        if gevonden:
            cnt    = gevonden.get("aantal_downloads", gevonden.get("downloads", 1))
            status = f"✓ Gedownload — eerste keer op {gevonden['eerste_download']} ({cnt}× totaal)"
            skl    = "#4a8a1a"
        else:
        ...
        <div class="zoek-result">
            <strong style="color:#e0f0ff;">Leerling {zoek}</strong><br>
            Verdachte arts in dataset: <span style="color:{kleur};font-weight:bold;">{arts}</span><br>
            Status: <span style="color:{skl};">{status}</span>
        </div>
        """, unsafe_allow_html=True)

        # Verwijder-knop (alleen tonen als leerling gevonden is)
        if gevonden:
            if st.button(f"🗑 Verwijder {zoek} uit de log", key="btn_verwijder"):
                verwijder_leerling(zoek)
                st.success(f"Leerling {zoek} is verwijderd uit de log.")
                st.rerun()

    st.markdown("</div>", unsafe_allow_html=True)

    # ── Export + uitloggen ──
    c_exp, c_uit = st.columns(2)
    with c_exp:
        st.download_button(
            "⬇ Export als CSV", data=log_als_csv(),
            file_name="sterfgeval_downloads.csv", mime="text/csv", key="export_csv",
        )
    with c_uit:
...
def render_week1():
    ...
    with c2:
        ...
        <div class="task-item"><span class="task-num">2</span><span>Bereken <strong>gemiddeld aantal patiënten</strong> per dienst met <code>=GEMIDDELDE(...)</code>.</span></div>
        <div class="task-item"><span class="task-num">3</span><span>Bepaal <strong>MAX en MIN</strong> overlijdens per dag.</span></div>
        <div class="task-item"><span class="task-num">4</span><span>Gebruik <strong>AutoFilter</strong> op Arts. Welke arts heeft de hoogste overlijdenskans?</span></div>
...
def render_week2():
    ...
    <div class="card"><div class="card-label">Verhaal</div>
        ...
        <p>Hoofdarts De Wit: <em>"Meer patiënten = meer risico, dus logisch dat het sterftecijfer hoger is."</em> Klopt dat statistisch?</p>
        <div class="clue-box"><span class="clue-label">🔍 Aanwijzing</span>
        Bereken overlijdens ÷ aantal patiënten. Als die verhouding bij één arts altijd hoger is — ook bij gelijk bezetting — dan klopt het argument niet.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c1:
        ...
        <div class="task-item"><span class="task-num">2</span><span>Bereken <strong>gem. quotiënt per arts</strong> met <code>=GEMIDDELDE.ALS()</code>.</span></div>
        <div class="task-item"><span class="task-num">3</span><span>Maak een <strong>draaitabel</strong>: rijen=Arts, kolommen=Dienst, waarden=gem. quotiënt.</span></div>
        <div class="task-item"><span class="task-num">4</span><span>Voeg een <strong>tijdlijnfilter</strong> toe op de datumkolom.</span></div>
    ...
    with c2:
        ...
        <div class="clue-box"><span class="clue-label">💡 Draaitabel tip</span>Zet het quotiënt op "Gemiddelde" (niet "Som") → Waardeveldinstellingen.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class="card"><div class="card-label">Voorbeeld</div>
        ...
        <div class="clue-box" style="margin-top:0.75rem;"><span class="clue-label">🔍 Patroon</span>
        Als de afwijking in álle diensten zichtbaar is, dan is de dienstroostering waarschijnlijk niet de oorzaak.</div>
    </div>
    """, unsafe_allow_html=True)
...
def render_week3():
    ...
    <div class="card"><div class="card-label">Verhaal</div>
        ...
        Is er statistisch bewijs dat één arts verantwoordelijk is voor de verhoogde sterfte? Of zijn er alternatieve verklaringen?</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="task-item"><span class="task-num">4</span><span><strong>Conclusie</strong> 150–250 woorden in tabblad 'Conclusie'. Gebruik de §1–§4 structuur.</span></div>
    ...
    <div class="card"><div class="card-label">Achtergrond</div>
        ...
        <p>Gebaseerd op de zaak van <strong>Lucia de Berk</strong> (2003–2010): verpleegkundige die veroordeeld werd op basis van statistisch bewijs — later bleek dit verkeerd geïnterpreteerd. In 2010 volledig vrijgesproken.</p>
